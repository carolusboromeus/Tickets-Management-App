...
import { getMasterData, getTicketDeleted, patchTicketDeleted } from '../lib/api';
...
const Layout = () => {
    ...
    const renderData = async (filter) => {
        const data = await getTicketDeleted(filter);
        // console.log(data);
            ...
            if (columnRefs[editableColumn] && !columnRefs[editableColumn].contains(event.target)) {
                ...
                if(formData[editableColumn]){
                    ...
                    try {
                        ...
                        const message = await patchTicketDeleted(updateData);
                        setMessage(message);
                    } finally {
                        setShowMessage(true);
    ...
    const handleExport = () => {
        ...
        epxortFiletoExcel(table, 'TicketDeleted', colWidth);
    };

    return (
        <>
            <div className="grid">
                <h1 className="text-xl font-semibold mb-4">Ticket Deleted</h1>
                            ...
                            <tr>
                                ...
                                {visibleColumns.week &&
                                    <th className="whitespace-nowrap px-4 py-2 cursor-pointer"
                                        ...
                                        <span className={`${showFilter.week ? "ml-6 mr-2" : "mx-6"}`}>Week</span>
                                        {showFilter.week && 
                                            <FilterOption
                                ...
                                {visibleColumns.client &&
                                    <th className="whitespace-nowrap px-4 py-2 cursor-pointer"
                                        ...
                                        <span className={`${showFilter.client ? "ml-10 mr-6" : "mx-10"}`}>Client</span>
                                        {showFilter.client && 
                                            <FilterInput
                                                filterOpen={filterOpen}
                                                setFilterOpen={setFilterOpen}
                                ...
                                {visibleColumns.resolvedBy && 
                                    ...
                                    >
                                        <span className={`${showFilter.resolvedBy ? "ml-10 mr-6" : "mx-10"}`}>Resolved By</span>
                                ...
                                {visibleColumns.requestType && 
                                    ...
                                    >
                                        <span className={`${showFilter.requestType ? "ml-8 mr-4" : "mx-8"}`}>Request Type</span>
                                ...
                                {visibleColumns.priority && 
                                    ...
                                    >
                                        <span className={`${showFilter.priority ? "ml-6 mr-2" : "mx-6"}`}>Priority</span>
                                        {showFilter.priority && 
                                            <FilterOption
                                                filterOpen={filterOpen}
                                                setFilterOpen={setFilterOpen}
